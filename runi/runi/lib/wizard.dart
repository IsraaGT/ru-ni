...
class _WizardScreenState extends State<WizardScreen> {
  final GeminiService _geminiService =
      GeminiService(apiKey: "AIzaSyD7ZUEYgkc8xQ_fECVu55hhNPD8pdXxBAE"); // REEMPLAZA CON TU API KEY REAL
  
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final FirebaseAuth _auth = FirebaseAuth.instance;

  // Definición de la nueva pregunta e inserción en la lista _questions
  // El índice de la nueva pregunta será 3. Las preguntas posteriores se desplazan.
  final List<Map<String, dynamic>> _questions = [
    {
      ...
      'type': 'project_name_text', // Índice 0
    },
    {
      'question': '¿Tu empresa tiene nombre?', // Índice 1
      'type': 'choice',
      'options': ['Sí', 'No'],
    },
    {
      'question': '¿Cuál es el giro de tu negocio?', // Índice 2
      'type': 'choice',
      'options': ['Comercial', 'Industrial', 'Servicios', 'No estoy seguro'],
    },
    // --- NUEVA PREGUNTA (NUEVO ÍNDICE 3) ---
    {
    ...
    // --- RESTO DE PREGUNTAS DESPLAZADAS ---
    {
      'question': '¿De qué tamaño consideras que es tu empresa?', // Antes índice 3, ahora 4
      'type': 'choice',
    ...
    {
      'question': '¿Por qué existe tu empresa?', // Antes índice 4, ahora 5
      'type': 'choice',
      'options': [
    ...
    {
      'question': '¿Qué te motivó a crearla?', // Antes índice 5, ahora 6
      'type': 'choice',
      'options': [
    ...
    {
      'question': '¿Qué problema resuelve tu empresa en el mercado?', // Antes índice 6, ahora 7
      'type': 'choice',
      'options': [
        'Falta de calidad',
        'Precios elevados',
  ...
  List<dynamic> _getCombinedAnswers() {
    ...
    // Para la nueva pregunta de descripción, si _answers[3] está actualizado, ya estará en 'combined'.
    // No se necesita lógica adicional aquí si _answers[3] se actualiza correctamente en onChanged del TextField.
  ...
  void dispose() {
    ...
    _descripcionEmpresaController.dispose(); // Disponer el nuevo controller
  ...
  String construirPrompt(List<dynamic> currentAnswers) {
    // Asegurarse de que currentAnswers tiene suficientes elementos para evitar errores de rango
    // Se usa .length > index para chequear y luego currentAnswers[index]
    return '''
