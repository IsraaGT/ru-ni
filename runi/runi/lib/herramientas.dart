...
// import 'package:flutter/services.dart'; // No se usa directamente en este archivo
import 'package:intl/intl.dart'; // No se usa directamente aquí, pero puede ser útil en otras partes
import 'package:internet_connection_checker/internet_connection_checker.dart';
import 'package:google_sign_in/google_sign_in.dart'; // No se usa directamente aquí
// Asegúrate que estas rutas sean correctas para tu estructura de proyecto
import 'package:runi/db.dart'; // No se usa directamente aquí
import 'package:runi/wizard.dart'; // No se usa directamente aquí, solo para el tipo WizardScreen
import 'package:url_launcher/url_launcher.dart'; // No se usa directamente aquí

// Screen principal que contiene el BottomNavigationBar y las diferentes vistas/pestañas
// Esta clase MainScreen parece estar bien y no necesita cambios para el problema actual.
// La mantengo por contexto del archivo.
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: BottomNavigationBar(
          items: const <BottomNavigationBarItem>[
            ...
            BottomNavigationBarItem(icon: Icon(Icons.build), label: "Generar Logo"), // Cambiado para reflejar la funcionalidad
...
// HomeContent parece estar bien y no necesita cambios para el problema actual.
// La mantengo por contexto del archivo.
...
class _SelectProjectForLogoPromptScreenState
  ...
  Future<void> _checkInternetAndLoadData() async {
    ...
    if (isConnected) {
      // Solo actualiza _hasInternet si realmente cambió para evitar rebuilds innecesarios
      if (!_hasInternet) {
        setState(() {
          _hasInternet = true;
        });
      }
      await _loadUserAndProjectsFromFirestore(); // Mover la carga aquí para que siempre intente cargar si hay internet
    } else {
      if (_hasInternet) { // Solo actualiza y muestra SnackBar si antes había internet
        setState(() {
          _hasInternet = false;
          _isLoading = false;
          _projects = [];
      ...
      } else { // Si ya no había internet, solo nos aseguramos que isLoading sea false
         setState(() {
          _isLoading = false;
          _projects = [];
        });
  ...
  Future<void> _loadUserAndProjectsFromFirestore() async {
    // Si no está montado o si no hay internet (doble chequeo), no continuar.
    // La verificación de internet principal ya se hizo en _checkInternetAndLoadData.
    if (!mounted || !_hasInternet) {
      // Si no hay internet pero se llamó esta función, asegurar que isLoading sea false.
    ...
    // Asegurar que isLoading esté true al inicio de la carga real de Firestore.
    // Esto es importante si se llama directamente a esta función (ej. desde el botón de recarga).
    if (!_isLoading) {
      setState(() => _isLoading = true);
    }
    ...
    if (user == null) {
      if (mounted) {
        ...
        // Opcional: Mostrar SnackBar aquí también si se desea, aunque ya se maneja en el build.
      }
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
    ...
    } else if (_firebaseAuthUid == null) { // Usuario no logueado
        bodyContent = _buildCenteredMessageWithButton(
        iconData: Icons.login_rounded,
        message: "Debes iniciar sesión",
        subMessage: "Inicia sesión desde la pestaña de Perfil para acceder a tus proyectos en la nube.",
        onButtonPressed: () {
            // Podríamos intentar recargar, pero es mejor que el usuario navegue a Perfil.
            // Opcionalmente, podríamos navegar directamente a la pestaña de perfil aquí.
            // Por ahora, solo recargamos para que el estado se re-evalúe.
             _checkInternetAndLoadData();
             ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
     ...
     else if (_projects.isEmpty) { // Usuario logueado, hay internet, pero no hay proyectos
      bodyContent = _buildCenteredMessageWithButton(
        ...
        onButtonPressed: _checkInternetAndLoadData, // O _loadUserAndProjectsFromFirestore directamente
        buttonText: "Recargar Proyectos"
      );
    }
     else { // Hay proyectos para mostrar
      bodyContent = Expanded(
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              // El botón de recarga solo se muestra si no estamos cargando Y hay conexión Y el usuario está logueado
              if (!_isLoading && _hasInternet && _firebaseAuthUid != null)
                Tooltip(
                  ...
                  child: IconButton(
                    ...
                    onPressed: _loadUserAndProjectsFromFirestore, // Llama directamente a la carga de Firestore
                    padding: EdgeInsets.all(12), // Aumentar el área táctil
                    splashRadius: 24,
                  ),
                ),
            ],
...
// --- Pantallas ProfileContent, SettingsContent, SettingsDetailPage ---
// Estas clases se mantienen como las tenías.
// Solo asegúrate que WizardScreen, LogoStyleQuestionScreen, ShowLogoPromptScreen estén definidas.

// Placeholder para LogoStyleQuestionScreen si no la tienes aún completamente definida
...
// Placeholder para ShowLogoPromptScreen si no la tienes aún completamente definida
...
// (Estas clases se omiten por brevedad, asumiendo que son las mismas que me proporcionaste antes
// y no necesitan cambios para la funcionalidad del botón de recarga en ToolsContent)
// Asegúrate de tenerlas definidas en tu proyecto. Si necesitas que las incluya, dímelo.
